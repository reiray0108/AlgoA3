...
// free the system's components of sokoban
void free_sokoban(sokoban_t sokoban) {
	for (int j = 0; j < sokoban.lines; j++) {
		if (sokoban.map[j]) free(sokoban.map[j]); 
		if (sokoban.map_save[j]) free(sokoban.map_save[j]);
	}
	if (sokoban.map) {
		if (sokoban.map_save) free(sokoban.map_save);
		if (sokoban.map) free(sokoban.map);
	}
	if (sokoban.buffer) free (sokoban.buffer);
}
